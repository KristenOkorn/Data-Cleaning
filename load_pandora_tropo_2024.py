    ...
    for n in range(len(pollutant)):
            ...
            if locations[k] == 'SJSU':
                ...
                if pollutant == 'NO2':
                    ...
                    desired_cols = [0, 55, 38, 56, 14, 64, 67, 13, 3, 64] + list(range(layer, len(data.columns)))
                    #datetime, no2, quality_flag, surf_uncert, temp, max_vert_tropo, top_height, pressure, SZA, max_width, layer1+

                elif pollutant == 'HCHO':
                    layer = 53 #get the index that the layers start at
                    desired_cols = [0, 44, 38, 45, 14, 51, 52, 13, 3, 64] + list(range(layer, len(data.columns)))
                    #datetime, hcho, quality_flag, surf_uncert, temp,  max_vert_tropo, top_height, pressure, SZA, horiz width, layer1+
            ...
            if locations[k] == 'SJSU':
                ...
                data.columns = ['datetime','{}'.format(pollutant),'quality_flag','surface_uncertainty','temperature','max_vert_tropo','top_height','pressure','SZA','max_horiz'] + layers
